...
router.post("/", (req, res, next) => {
  ...
  try {
    ...
    db = JSON.parse(db);
    const { books } = db;
    //add new books to JS object
    books.push(newBook);
    // add new book to db JS object
    db.books = books;
    //db JS object to Json String
...
router.put("/:bookId", (req, res, next) => {
  ...
  try {
    ...
    const notAllow = updateKeys.filter(
      (updateKey) => !allowUpdate.includes(updateKey)
    );
    if (notAllow.length) {
      const exception = new Error("Update field is not allowed");
      throw exception;
    }
...
router.delete("/:bookId", (req, res, next) => {
  try {
    ...
    //db JSobject to JSON string
    db = JSON.stringify(db);
    ////write and save to JSON string
    fs.writeFileSync("db.json", db);

    //deleted sending response
    res.status(200).send({});
  } catch (error) {
    next(error);
  }
